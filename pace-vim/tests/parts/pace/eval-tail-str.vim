...
Assert_Equal(501, [-1, -1], Reltime([0, 0], [0, -1]))
...
try
	...
	Assert_Equal(97, (12 * 1), pace.carry)

	mockup.mode = 'n'
	const eval_2: number = Pace_Load(0)
	Assert_Equal(98, 0, pace.carry)

	const eval_3: number = Pace_Load(1)
	mockup.mode = 'i'
	unlet! g:pace_info g:pace_policy
	g:pace_policy = 10007
	Assert_Equal(99, -1, Get_Chars())
	Assert_Equal(100, -1, Get_Secs())
	...
	mockup.time.after = [0, 166667]	# ≈1/6
	Set_Secs(0)
####	Set_Parts(pace.carry)

	doautocmd pace CursorMovedI
	Assert_Equal(101, 0, Get_Secs())
	Assert_Equal(102, 166667, Get_Parts())
	doautocmd pace CursorMovedI
	Assert_Equal(103, 0, Get_Secs())
	Assert_Equal(104, 333334, Get_Parts())
	doautocmd pace CursorMovedI
	Assert_Equal(105, 0, Get_Secs())
	Assert_Equal(106, 500001, Get_Parts())
	doautocmd pace CursorMovedI
	Assert_Equal(107, 0, Get_Secs())
	Assert_Equal(108, 666668, Get_Parts())
	doautocmd pace CursorMovedI
	Assert_Equal(109, 0, Get_Secs())
	Assert_Equal(110, 833335, Get_Parts())
	doautocmd pace CursorMovedI
	Assert_Equal(111, 1, Get_Secs())	# (1000002 / 1000000)
	Assert_Equal(112, 2, Get_Parts())	# (1000002 % 1000000)
	Assert_Equal(113, mockup.time.before, Get_Tick())
	Assert_Equal(114, 6, Get_Chars())
	Assert_True(31, exists('g:pace_info'))
	eval_any = matchlist(g:pace_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
	Assert_Equal(115, '0.16', eval_any[1])
	Assert_Equal(116, '6', eval_any[2])
	Assert_Equal(117, '6', eval_any[3])
	Assert_Equal(118, '1', eval_any[4])
	doautocmd pace CursorMovedI
	Assert_Equal(119, mockup.time.before, Get_Tick())
	Assert_Equal(120, 7, Get_Chars())
	Assert_Equal(121, 1, Get_Secs())
	Assert_True(32, exists('g:pace_info'))
	eval_any = matchlist(g:pace_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
	Assert_Equal(122, '0.16', eval_any[1])
	Assert_Equal(123, '7', eval_any[2])
	Assert_Equal(124, '7', eval_any[3])
	Assert_Equal(125, '1', eval_any[4])

	Assert_True(33, exists('#pace#InsertLeave#*'))
	doautocmd pace InsertLeave
	Assert_Equal(126, (166667 + 2), pace.carry)
	mockup.mode = 'n'
	const eval_4: number = Pace_Load(0)
	Assert_Equal(127, 0, pace.carry)

	const eval_5: number = Pace_Load(1)
	mockup.mode = 'i'
	unlet! g:pace_info g:pace_policy g:pace_sample
	g:pace_policy = 10007
	Assert_Equal(128, -1, Get_Chars())
	Assert_Equal(129, -1, Get_Secs())
	...
	try
		...
		doautocmd pace InsertEnter
		Assert_Equal(130, &updatetime, pace.sample_in)
		Assert_True(37, exists('g:pace_info'))
		eval_any = matchlist(g:pace_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
		Assert_Equal(131, '0.00', eval_any[1])
		Assert_Equal(132, '0', eval_any[2])
		Assert_Equal(133, '0', eval_any[3])
		Assert_Equal(134, '0', eval_any[4])

		Assert_Equal(135, 0, pace.carry)
		Assert_True(38, exists('#pace#CursorMovedI#*'))
		mockup.time.after = [0, 166667]	# ≈1/6
		Set_Secs(0)

		doautocmd pace CursorMovedI
		Assert_Equal(136, 0, Get_Secs())
		Assert_Equal(137, 166667, Get_Parts())
		doautocmd pace CursorMovedI
		Assert_Equal(138, 0, Get_Secs())
		Assert_Equal(139, 333334, Get_Parts())
		doautocmd pace CursorMovedI
		Assert_Equal(140, 0, Get_Secs())
		Assert_Equal(141, 500001, Get_Parts())
		doautocmd pace CursorMovedI
		Assert_Equal(142, 0, Get_Secs())
		Assert_Equal(143, 666668, Get_Parts())
		doautocmd pace CursorMovedI
		Assert_Equal(144, 0, Get_Secs())
		Assert_Equal(145, 833335, Get_Parts())
		doautocmd pace CursorMovedI
		Assert_Equal(146, 1, Get_Secs())
		Assert_Equal(147, 2, Get_Parts())
		Assert_Equal(148, mockup.time.before, Get_Tick())
		Assert_Equal(149, 6, Get_Chars())
		Assert_True(39, exists('g:pace_info'))
		eval_any = matchlist(g:pace_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
		Assert_Equal(150, '0.00', eval_any[1])
		Assert_Equal(151, '0', eval_any[2])
		Assert_Equal(152, '0', eval_any[3])
		Assert_Equal(153, '0', eval_any[4])
		Assert_True(40, exists('#pace#CursorHoldI#*'))
		doautocmd pace CursorHoldI
		Assert_True(41, exists('g:pace_info'))
		eval_any = matchlist(g:pace_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
		Assert_Equal(154, '0.00', eval_any[1])
		Assert_Equal(155, '6', eval_any[2])
		Assert_Equal(156, '6', eval_any[3])
		Assert_Equal(157, '1', eval_any[4])

		Assert_True(42, exists('#pace#CursorMovedI#*'))
		doautocmd pace CursorMovedI
		Assert_True(43, exists('#pace#CursorHoldI#*'))
		doautocmd pace CursorHoldI
		Assert_Equal(158, mockup.time.before, Get_Tick())
		Assert_Equal(159, 7, Get_Chars())
		Assert_Equal(160, 1, Get_Secs())
		Assert_True(44, exists('g:pace_info'))
		eval_any = matchlist(g:pace_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
		Assert_Equal(161, '0.00', eval_any[1])
		Assert_Equal(162, '7', eval_any[2])
		Assert_Equal(163, '7', eval_any[3])
		Assert_Equal(164, '1', eval_any[4])

		Assert_True(45, exists('#pace#InsertLeave#*'))
		doautocmd pace InsertLeave
		Assert_Equal(165, (166667 + 2), pace.carry)
		mockup.mode = 'n'
		const eval_6: number = Pace_Load(0)
		Assert_Equal(166, 0, pace.carry)
	finally
