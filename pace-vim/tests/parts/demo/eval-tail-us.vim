...
try
	...
	call s:Assert_Equal(45, s:mockup.time.before, s:Get_Tick())
	call s:Assert_Equal(46, 11, s:Get_Chars())
	call s:Assert_Equal(47, 1, s:Get_Secs())
	call s:Assert_True(6, exists('g:demo_info'))
	let s:eval_any = matchlist(g:demo_info,
				\ '\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
	call s:Assert_Equal(48, '0.16', s:eval_any[1])
	call s:Assert_Equal(49, '11', s:eval_any[2])
	call s:Assert_Equal(50, '11', s:eval_any[3])
	call s:Assert_Equal(51, '1', s:eval_any[4])
finally
