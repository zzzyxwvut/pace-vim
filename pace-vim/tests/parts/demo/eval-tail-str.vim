...
try
	...
	Assert_Equal(32, 166667, Get_Parts())
	Eval0(turn)
	Assert_Equal(33, 0, Get_Secs())
	Assert_Equal(34, 333334, Get_Parts())
	Eval0(turn)
	Assert_Equal(35, 0, Get_Secs())
	Assert_Equal(36, 500001, Get_Parts())
	Eval0(turn)
	Assert_Equal(37, 0, Get_Secs())
	Assert_Equal(38, 666668, Get_Parts())
	Eval0(turn)
	Assert_Equal(39, 0, Get_Secs())
	Assert_Equal(40, 833335, Get_Parts())
	Eval0(turn)
	Assert_Equal(41, 1, Get_Secs())		# (1000002 / 1000000)
	Assert_Equal(42, 2, Get_Parts())	# (1000002 % 1000000)
	Assert_Equal(43, mockup.time.before, Get_Tick())
	Assert_Equal(44, 6, Get_Chars())
	Assert_True(6, exists('g:demo_info'))
	eval_any = matchlist(g:demo_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
	Assert_Equal(45, '0.16', eval_any[1])
	Assert_Equal(46, '6', eval_any[2])
	Assert_Equal(47, '6', eval_any[3])
	Assert_Equal(48, '1', eval_any[4])
	Eval1(turn)
	Assert_Equal(49, mockup.time.before, Get_Tick())
	Assert_Equal(50, 7, Get_Chars())
	Assert_Equal(51, 1, Get_Secs())
	Assert_True(7, exists('g:demo_info'))
	eval_any = matchlist(g:demo_info,
				'\v(\d+\.\d\d),\s+(\d+),\s+(\d+),\s+(\d+)')
	Assert_Equal(52, '0.16', eval_any[1])
	Assert_Equal(53, '7', eval_any[2])
	Assert_Equal(54, '7', eval_any[3])
	Assert_Equal(55, '1', eval_any[4])
finally
