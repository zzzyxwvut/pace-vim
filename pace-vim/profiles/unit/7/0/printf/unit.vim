" Description:	A _printf_ runner for the "Eval*" functions (Vim 7.0)
...
function! Unit_Runner_PRF_7_0(fname, lines, times, delay) abort
	return 0
endfunction

	call s:runner.errmsg("Unsupported for parts: ".s:parts)
	let &cpoptions = s:cpoptions
	unlet s:parts s:runner s:cpoptions
	finish
endif

if empty($UNIT_PROFILE_TRACE) || !(s:parts == 6 || s:parts == 9)
...
function! s:runner.trace(go) abort					" {{{1
	let l:tick = reltime(a:go.a)
	call add(l:self.dump,
		\ [l:tick, l:tick[0].(printf('.%06i', l:tick[1]))[: 2]])
endfunction								" }}}1
...
function! s:runner.trace(go) abort					" {{{1
	let l:tick = reltime(a:go.a)
	call add(l:self.dump,
		\ [l:tick, l:tick[0].(printf('.%09i', l:tick[1]))[: 2]])
endfunction								" }}}1

endif

function! s:runner.report() abort					" {{{1
	call writefile(map(l:self.dump[:], 'string(v:val)'), 'unit_printf.vim')
...
function! Unit_Runner_PRF_7_0(fname, lines, times, delay) abort		" {{{1
