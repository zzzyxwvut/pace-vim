" Description:	A top-bottom runner for the "eval*" functions (Vim 7.0)
...
function! s:Eval1(self) abort						" {{{1
	...
	let a:self.char = a:self.char + 1
	let a:self.sec = l:tick[2]
...
function! Eval_Runner_TB_7_0(times, fname, ...) abort			" {{{1
